...
- **Cómo propagan los cambios:** el plugin se instala **una sola vez a nivel usuario**, pero Claude Code lo sirve desde una **caché versionada** (`~/.claude/plugins/cache/federicode/harness/<version>`), no en vivo desde esta carpeta. Esa `<version>` es la clave del caché, y se resuelve así: `version` de `plugin.json` → `version` de la entrada del marketplace → **el SHA del commit**. Ninguno de los dos JSON declara `version` a propósito: cada commit acá **es** una versión nueva, y el ciclo queda en dos pasos — (1) commit, (2) `/plugin update harness@federicode`. Sin campos que bumpear a mano ni caché que borrar. (Excepción: `format.md` no pasa por el caché — se lee en vivo, ver *Estructura*.)
- **Nunca vuelvas a declarar `version`** en `plugin.json` ni en la entrada del marketplace. Un `version` fijo **pinea** el plugin: los commits nuevos dejan de propagarse sin que nada avise, y vuelve el ritual de bump + borrar `~/.claude/plugins/cache/federicode/` + reiniciar. La doc además desaconseja declararlo en los dos lados, porque el de `plugin.json` gana en silencio.
- **El repo git de esta carpeta es infraestructura, no prolijidad:** sin commits no hay SHA, y sin SHA no hay versión que propagar. Un working tree sucio es harness que no corre en ninguna sesión.
