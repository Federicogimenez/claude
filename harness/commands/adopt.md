...
- Agregá `.scratch/` al `.gitignore` del workspace: es la ubicación canónica de los artefactos derivados de sesión (ver *Reglas transversales* en `format.md`). **No la crees vacía** — la crea la primera sesión que la necesite; lo que se siembra es el ignore, para que su contenido nunca entre a un commit.
